...
def timbral_reverb(
    fname, fs=0, dev_output=False, phase_correction=False, clip_output=False
):
    """
    ...
    raw_audio_samples, fs = timbral_util.file_read(
        fname, fs=fs, phase_correction=False, mono_sum=False, loudnorm=False
    )
...
def estimate_RT60(audio_samples, fs):
    ...
    par = init_rt_estimate_e(
        fs
    )  # struct with all parameters and buffers for frame-wise processing
...
def init_rt_estimate_e(fs=24000):
    ...
    no = (
        par["fs"] / 24000.0
    )  # correction factor to account for different sampling frequency

    # pararmeters for pre - selection of suitable segments
    if par["fs"] > 8e3:
        par[
            "down"
        ] = 2  # rate for downsampling applied before RT estimation to reduce computational complexity
    else:
        par["down"] = 1

    par["N_sub"] = int(
        round(no * 700 / par["down"])
    )  # sub-frame length(after downsampling)
    par["N_shift"] = int(
        round(no * 200 / par["down"])
    )  # frame shift(before downsampling)
    ...
    par["N"] = int(
        par["nos_max"] * par["N_sub"]
    )  # maximal frame length(after downsampling)
    ...
    par["buffer"] = np.zeros(
        par["buffer_size"]
    )  # buffer with previous indices to update histogram
    ...
    par["RT_raw"] = par[
        "RT_initial"
    ]  # raw RT estimate obtained by histogram - approach
...
def rt_estimate_frame_my(frame, par):
    ...
    for k in range(2, par["nos_max"]):
        ...
        if (var_pre > var_cur) and (max_pre > max_cur) and (min_pre < min_cur):
            ...
            if cnt >= par["nos_min"]:
                ...
                RTml, _ = max_loglf(
                    frame[: cnt * par["N_sub"]], par["a"], par["Tquant"]
                )
    ...
    if RTml >= 0:  # new ML estimate calculated
        ...
        if par["hist_counter"] > par["buffer_size"] + 1:
            ...
            par["hist_rt"][int(par["buffer"][0])] = (
                par["hist_rt"][int(par["buffer"][0])] - 1
            )

        par["buffer"] = np.append(
            par["buffer"][1:], index
