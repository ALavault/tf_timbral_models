...
from . import (tf_timbral_booming, tf_timbral_brightness_2, tf_timbral_depth,
               tf_timbral_hardness, tf_timbral_roughness, tf_timbral_sharpness,
               tf_timbral_warmth, timbral_booming, timbral_brightness,
               timbral_depth, timbral_hardness, timbral_reverb,
               timbral_roughness, timbral_sharpness, timbral_util,
               timbral_warmth)
...
def timbral_extractor(fname, fs=0, dev_output=False, phase_correction=False, clip_output=False, output_type='dictionary', verbose=True):
    ...
    if output_type == 'dictionary':
        ...
        timbre = [hardness, depth, brightness, roughness,
...
def tf_timbral_extractor(audio_samples, fs=0, descriptors=["brightness"]):
    # ['brightness', 'hardness', 'depth', 'roughness', 'booming', 'warmth', 'sharpness']
    if isinstance(descriptors, str) and descriptors == 'all':
        descriptors = ['brightness', 'hardness', 'depth',
                       'roughness', 'booming', 'warmth', 'sharpness']
    out = []
    if "brightness" in descriptors:
        out.append(tf_timbral_brightness_2(audio_samples, fs))
    if "hardness" in descriptors:
        out.append(tf_timbral_hardness(audio_samples, fs))
    if "depth" in descriptors:
        out.append(tf_timbral_depth(audio_samples, fs))
    if "roughness" in descriptors:
        out.append(tf_timbral_roughness(audio_samples, fs))
    if "boominess" in descriptors:
        out.append(tf_timbral_booming(audio_samples, fs))
    if "warmth" in descriptors:
        out.append(tf_timbral_warmth(audio_samples, fs))
    if "sharpness" in descriptors:
        out.append(tf_timbral_sharpness(audio_samples, fs))
    out = tf.stack(out)
    # probably (n_descriptors, batch,1), wanted : (batch, n_descriptors)
    print(out.shape)
    return out
