...
def boominess_calculate(loudspec):
    ...
    #print(Bandsum, Ll, loudness)
    BoomingIndex = Bandsum * (Ll / loudness)

    return BoomingIndex


def timbral_booming(fname, fs=0, dev_output=False, phase_correction=False, clip_output=False, take_first=None):
    """
    ...
    # The window audio function is weird
    if take_first:
    ...
    #print("windowed_rms", windowed_rms.shape)
    # get the weighted average
    rms_boom = np.average(windowed_booming, weights=(
        windowed_rms * windowed_rms))
    ...
    if dev_output:
        ...
        coefficients = np.array(
            [43.67402696195865, -10.90054738389845, 26.836530575185435])
...
def tf_timbral_booming(audio_samples, fs=0, dev_output=False, phase_correction=False, clip_output=False):
    ...
    for k in range(b):
        windowed_booming = []
        for i in range(windowed_audio.shape[1]):
            samples = windowed_audio[k, i, :]  # the current time window
            ...
            if N_entire > 0:
                BoomingIndex = tf.numpy_function(
                    boominess_calculate, [N_single], [tf.float64])
            else:
                BoomingIndex = [tf.cast(0., tf.float64)]
            windowed_booming.append(BoomingIndex)

        # get level of low frequencies
        ll, _ = tf.numpy_function(
            timbral_util.weighted_bark_level,
            [audio_samples[k], fs, 0, 70],
            [tf.float64, tf.float64],
        )
        #windowed_booming = np.array(windowed_booming)
        # ll, w_ll = timbral_util.weighted_bark_level(audio_samples, fs, 0, 70)
        ll = timbral_util.tf_log10(ll)
        ll_array.append(ll)
        # convert to numpy arrays for fancy indexing
        # windowed_booming = np.array(windowed_booming)
        # get the weighted average
        # print(windowed_booming)
        windowed_booming = tf.squeeze(
            tf.stack(tf.cast(
                windowed_booming, audio_samples.dtype)))

        # print("shapes ::", windowed_booming, windowed_rms[k])
        #out = K.sum(x * y, axis=axis) / K.sum(y, axis=axis)

        rms_boom = timbral_util.tf_average(windowed_booming,
                                           windowed_rms[k] * windowed_rms[k], axis=0)
        rms_boom = timbral_util.tf_log10(rms_boom)
        rms_boom_array.append(rms_boom)

    ll = tf.cast(
        tf.stack(ll_array), rms_boom.dtype)
    rms_boom = tf.stack(rms_boom_array)
    if dev_output:
        return [rms_boom, ll]
    else:
        #print('rms_boom :: ', rms_boom, "ll :: ", ll)

        # perform thye linear regression
        # boominess = np.sum(all_metrics * coefficients)
        boominess = 43.67402696195865*rms_boom - \
            10.90054738389845 * ll + 26.836530575185435
